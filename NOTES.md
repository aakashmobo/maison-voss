...
- Hero (reverted to pre-redesign): full-width single-column layout — massive "SILENCE" headline left-aligned, editorial portrait floated to the right (absolute, 38% width, fading into black on left edge), thin horizontal rule through the letterforms, no column border-right
- "NOW ARRIVING" lime pill badge sits top-right inside the portrait overlay
...
- Hero: reverted to pre-redesign stage (single-column, portrait floated right)
...
## Sections Built
1. Hero — reverted to pre-redesign: single-column layout, portrait floated right, massive SILENCE, rule through letters
